...
// 解析 m3u 内容，提取 group-title="正在直播" 的条目，转换为标准 JSON
async function fetchAndConvertPPVM3U() {
  const url = 'https://ppv.hxfkof.dpdns.org/ppv.php?id=list';
        ...
        try {
          ...
          for (let i = 0; i < lines.length; i++) {
            ...
            if (line.startsWith('#EXTINF:')) {
              ...
              // 提取逗号后的描述
              const descStart = line.lastIndexOf(',');
              if (descStart === -1) {
                currentExtInf = null;
                continue;
              }
              const description = line.substring(descStart + 1).trim();
              ...
              // 这是播放链接行
              const playUrl = line;
              const desc = currentExtInf.description;

              // 按 " - " 分割，最多3部分
              const parts = desc.split(/\s*-\s*/);
              let dateTimeStr = parts[0] || '';      // 例如 "06/05 10:00"
              let pkInfo = parts[1] || '';           // 例如 "England vs. New Zealand"
              let label = parts[2] || '';             // 例如 "Fox Cricket"
              
              // 如果分割后只有两个部分，则第三个为空
              // 对日期时间格式转换 MM/DD HH:MM -> MM月DD日HH:MM
              let formattedTime = '';
              ...
              const matchItem = {
                ...
                padImg: '',
                competitionLogo: currentExtInf.tvgLogo,
                pkInfoTitle: pkInfo,
                modifyTitle: label,
                presenters: '',
                matchInfo: { time: formattedTime },
                nodes: [
                  {
                    pID: playUrl,
                    name: pkInfo
              ...
              currentExtInf = null; // 重置
            }
          }

          console.log(`PPV m3u 解析完成，共获取 ${matches.length} 个正在直播的条目`);
          resolve(matches);
        } catch (error) {
          console.error('解析 PPV m3u 内容失败:', error.message);
          resolve([]);
        }
      });
    }).on('error', (err) => {
      console.error('请求 PPV m3u 列表失败:', err.message);
      resolve([]);
    }).on('timeout', () => {
      console.error('请求 PPV m3u 列表超时');
      resolve([]);
    });
  });
}
  ...
  try {
    ...
    if (mode === 'today') {
      ...
      try {
        ...
        if (ppvMatches.length > 0) {
          console.log(`添加 ${ppvMatches.length} 个 PPV 直播条目`);
          mergedMatches = mergedMatches.concat(ppvMatches);
        } else {
          console.log('未获取到 PPV 直播数据或列表为空');
        }
      } catch (ppvError) {
